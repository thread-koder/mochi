...
import (
	...
	"github.com/thread_koder/mochi/core/internal/disk"
)

// AnalyzeNamespace runs disk analysis for all workloads in a namespace.
func AnalyzeNamespace(c *gin.Context) {
	...
	analysis, err := disk.AnalyzeNamespace(ctx, namespace, opts)
	if err != nil {
		c.Error(err)
...
// AnalyzeWorkload runs disk analysis for one workload or standalone pod.
func AnalyzeWorkload(c *gin.Context) {
	...
	analysis, err := disk.AnalyzeWorkload(ctx, workloadType, workloadName, namespace, pods, opts, true)
	if err != nil {
		c.Error(err)
		c.JSON(http.StatusInternalServerError, gin.H{
